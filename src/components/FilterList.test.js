...
test('filter label exists', () => {
  const { queryByTestId } = render(<FilterList selected={[...testCategories]} onRemove={jest.fn()} />)
  const listLabel = queryByTestId('filter-listLabel')
  expect(listLabel).toBeInTheDocument()
})

test('if no filter is selected, no entries are rendered', () => {
  const { queryByTestId } = render(<FilterList selected={[]} onRemove={jest.fn()} />)
...
test('if 3 filters are selected, 3 entries are rendered', () => {
  const { queryAllByTestId } = render(<FilterList selected={[...testCategories]} onRemove={jest.fn()} />)
...
test('if a filter is selected, its name is rendered', () => {
  const { getByText } = render(<FilterList selected={[{ id: 3, name: 'salads'}]} onRemove={jest.fn()} />)
  const entry = getByText(/salads/i)
  expect(entry).toBeInTheDocument()
})

test('if a filter is selected, it has a remove button', () => {
  const { queryByTestId } = render(<FilterList selected={[{ id: 3, name: 'salads'}]} onRemove={jest.fn()} />)
...
test('when deletebutton is pressed, onRemove is called', () => {
  ...
  const { queryByTestId } = render(<FilterList selected={[{ id: 3, name: 'salads'}]} onRemove={deleteFunction} />)
  const removeButton = queryByTestId('filter-listEntry-deleteButton')
